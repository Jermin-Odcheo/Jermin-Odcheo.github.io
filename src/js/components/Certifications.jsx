  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        <Motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-8" variants={staggerContainer}>
              ...
              <div className="bg-[#374151]/20 backdrop-blur-sm rounded-xl overflow-hidden border border-[#6b7280]/30 hover:border-[#9ca3af]/50 hover:bg-[#374151]/30 transition-all duration-300 h-full flex flex-col">
                ...
                <div className="p-4 sm:p-6 flex-1 flex flex-col">
                  <div className="flex items-center mb-3">
                    <div
                      className={`w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-gradient-to-br flex items-center justify-center shadow-sm flex-shrink-0`}
                      style={{ backgroundImage: `linear-gradient(135deg, ${cert.colors[0]}, ${cert.colors[1]})` }}
                      aria-hidden="true"
                    >
                      <i className={`${cert.icon} text-white text-sm sm:text-base`}></i>
                    </div>
                    <div className="ml-3">
                      <h3 className="text-base sm:text-lg font-bold text-[#f9fafb] leading-tight">{cert.title}</h3>
                      <p className="text-[#9ca3af] text-xs sm:text-sm">{cert.platform}</p>
                    </div>
                  </div>

                  <p className="text-[#9ca3af] text-xs sm:text-sm flex-1">{cert.description}</p>
      ...
      {previewCert && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-60 p-3 sm:p-4" onClick={() => setPreviewCert(null)}>
          <div className="bg-[#1f2937] border border-[#4b5563] rounded-xl w-full max-w-3xl max-h-[90vh] sm:max-h-[80vh] overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 border-b border-[#374151] bg-[#111827]">
              <div>
                <h4 className="text-[#f9fafb] font-semibold text-sm sm:text-base">{previewCert.title}</h4>
                <p className="text-[#9ca3af] text-xs">{previewCert.platform}</p>
              </div>
              <button
                onClick={() => setPreviewCert(null)}
                className="text-[#9ca3af] hover:text-[#f9fafb] p-2 rounded-lg hover:bg-[#374151]/50 transition-colors ml-2 flex-shrink-0"
                aria-label="Close preview"
              >
                <i className="fas fa-times"></i>
              </button>
            </div>
            <div className="bg-[#111827] flex items-center justify-center p-2 sm:p-3 overflow-auto">
              <img
